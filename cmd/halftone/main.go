...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "Convert one or more fingerprint SVG documents in to dithered (halftone) JPEG images.\n\n")
	...
	for _, path := range flag.Args() {
		...
		out_fname := strings.Replace(fname, ".svg", "-halftone.jpg", 1)
		...
		_, err = fingerprint.Halftone(r, wr, max_dimension)
